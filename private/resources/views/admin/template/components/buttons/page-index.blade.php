<div style="margin-bottom: 30px;">
    ...
    @if (auth()->user()->can('admin.'.$modelMeta->name.'.edit'))
        ...
        @elseif (Route::has('admin.'.$modelMeta->name.'.gallery-sort') && Route::currentRouteName() == 'admin.'.$modelMeta->name.'.gallery-index')
            <a href="{{ route('admin.' . $modelMeta->name . '.gallery-sort', [
                $modelMeta->name => $row,
                'group' => request()->input('group', 'default')
            ]) }}" class="btn red-flamingo"><i class="fa fa-reorder"></i> Sırala</a>
        @endif
        ...
        @if (Route::currentRouteName() == 'admin.'.$modelMeta->name.'.gallery-sort')
            <a href="{{ $row->getGalleryLink(request()->input('group', 'default')) }}" class="btn blue-ebonyclay"><i class="fa fa-undo"></i> Galeriye Geri Dön</a>
        @endif
    @endif
