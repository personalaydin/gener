...
trait GalleryTrait
{
    ...
    public function setGalleryImage($file, $name = 'image')
    {
        // First create necessary folders
        $this->createImageFolders($name);

        // Upload images
        return $this->createGalleryImages($file, $name);
    }
    ...
    public function createGalleryImages($file, $name = 'image', $saveGalleryImageToDB = true)
    {
        // If hasn't file, exit
        if (!$file) {
            return;
        ...
        if ($saveGalleryImageToDB) {
            $gallery = $this->saveGalleryImageToDB($name, $fileName);
        }
    ...
    public function saveGalleryImageToDB($name, $fileName)
    {
        if (is_null($this)) {
            return;
        }
