...
use App\Models\Entities\PageGallery;
...
class PageController extends AdminController
{
    ...
    public function galleryIndex($id)
    {
        ...
        return parent::galleryIndex($row);
    }
    ...
    public function galleryStore(Request $request, $id)
    {
        ...
        foreach ($request->galleryImages as $file) {
            $gallery = PageGallery::create([
                'entity_id' => $id,
                'group' => $request->input('group', 'default'),
            ]);

            $response['files'][] = $gallery->setGalleryImage($file);
        }
     ...
     *
     * @param $id
     * @param null $galleries
     * @return Response
     */
    public function gallerySort($id, $galleries = null)
    {
        $row = Page::withTrashed()->findOrFail($id);
        $galleries = PageGallery::withDepth()->defaultOrder()->get()->toTree();

        return parent::gallerySort($row, $galleries);
    }
     ...
     * @return Response
     */
    public function galleryRebuildTree(Request $request)
    {
        $nestedSet = json_decode($request->nestable_list, true);

        PageGallery::rebuildTree($nestedSet);

        $this->generateFlashMessage('success', 'Sıralamalar güncellendi!');
     ...
     * @return Response
     */
    public function galleryHardDelete($id)
    {
        $gallery = PageGallery::findOrFail($id);

        return parent::galleryHardDelete($gallery);
    }
}
